...
From this the password is found: <i><b><span style="color:orange">santapass</span></i><br>
...
#### <u>Door app</u> ####
Running strings against the door, password is reveal: <i><b><span style="color:orange">Op3nTheD00r</span></i><br>
...
After some trial and error, the password is revealed: <i><b><span style="color:orange">CandyCane1</span></i><br>
...
<i><b><span style="color:orange">\d</span></i><br>
<i><b><span style="color:orange">[azAZ]</span></i><br>
<i><b><span style="color:orange">[a-z0-9]{2}</span></i><br>
<i><b><span style="color:orange">^(?!.*?[A-L1-5]).*</span></i><br>
<i><b><span style="color:orange">^[0-9]{3,}$</span></i><br>
<i><b><span style="color:orange">^(((([0-1][0-9])|(2[0-3])):?[0-5][0-9]:?[0-5][0-9]+$))</span></i><br>
<i><b><span style="color:orange">^[a-fA-F0-9]{2}(:[a-fA-F0-9]{2}){5}$</span></i><br>
FIXXX
<i><b><span style="color:orange">^(0[1-9]|1[0-4])[\/.-](0[1-9]|1\d|2\d|3[01]\)\[\/.-](19|20)\d{2}$</span></i><br>
Challenge solved<br>
...
With a timestamp of 1608926671.122520, just submit <i><b><span style="color:orange">122520</span></i> to answer<br>
...
Answer is found in /proc/self/environ of <i><b><span style="color:orange">JackFrostWasHere</span></i><br>
...
15 Scapy questions followed by answers in <i><b><span style="color:orange">orange</span></b>:
1. Start by running the task.submit() function passing in a string argument of 'start'.
<i><b><span style="color:orange">task.submit('start')</span></i>
2. Submit the class object of the scapy module that sends packets at layer 3 of the OSI model.
<i><b><span style="color:orange">task.submit(send)</span></i>
3. Submit the class object of the scapy module that sniffs network packets and returns those packets in a list.
<i><b><span style="color:orange">task.submit(sniff)</span></i>
4. Submit the NUMBER only from the choices below that would successfully send a TCP packet and then return the first sniffed response packet to be stored in a variable named "pkt": <i><b><span style="color:orange">task.submit(1)</span></i>
...
<i><b><span style="color:orange">task.submit(rdpcap)</span></i>
6. The variable UDP_PACKETS contains a list of UDP packets. Submit the NUMBER only from the choices below that correctly prints a summary of UDP_PACKETS:<i><b><span style="color:orange">task.submit(2)</span></i>
	1. UDP_PACKETS.print()
	2. UDP_PACKETS.show()
	3. UDP_PACKETS.list()
7. Submit only the first packet found in UDP_PACKETS.
<i><b><span style="color:orange">task.submit(UDP_PACKETS[0])</span></i>
8. Submit only the entire TCP layer of the second packet in TCP_PACKETS.
<i><b><span style="color:orange">pkt = TCP_PACKETS[1]</span></i>
<i><b><span style="color:orange">task.submit(pkt[TCP])</span></i>
...
<i><b><span style="color:orange">pkt[IP].src = '127.0.0.1'</span></i>
<i><b><span style="color:orange">task.submit(pkt[IP])</span></i>
10. Submit the password "task.submit('elf_password')" of the user alabaster as found in the packet list TCP_PACKETS.
<i><b><span style="color:orange">TCP_PACKETS[6] </span></i>
<i><b><span style="color:orange">task.submit('echo')</span></i>
...
<i><b><span style="color:orange">ICMP_PACKETS[1][ICMP].chksum</span></i>
<i><b><span style="color:orange">task.submit(19524)</span></i>
12. Submit the number of the choice below that would correctly create a ICMP echo request packet with a destination IP of 127.0.0.1 stored in the variable named "pkt" <i><b><span style="color:orange">task.submit(3)</span></i>
...
<i><b><span style="color:orange">pkt = IP(dst='127.127.127.127')/UDP(dport=5000)</span></i>
...
<i><b><span style="color:orange">pkt = IP(dst='127.2.3.4')/UDP(dport=53)/DNS(rd=1,qd=DNSQR(qname='elveslove.santa'))</span></i>
...
<i><b><span style="color:orange">pkt = ARP_PACKETS[1]</span></i>
<i><b><span style="color:orange">pkt[ARP].hwsrc='00:13:46:0b:22:ba'</span></i>
<i><b><span style="color:orange">pkt[ARP].hwdst='00:16:ce:6e:8b:24'</span></i>
<i><b><span style="color:orange">task.submit(ARP_PACKETS)</span></i><br>
...
Reading the output of the document, <i><b><span style="color:orange">Tanta Kringle</span></b> is the answer for the objective.<br>
...
<i><b><span style="color:orange">sha256sum * | grep 58a3b9335a6ceb0234c12d35a0564c4e</span></b><br>
...
<i><b><span style="color:orange">python3 naughty_nice.py | grep -A 20 129459 > 129459.block</span></b><br>
